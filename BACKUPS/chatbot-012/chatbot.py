...
system_prompt_chat = (
    ...
    "If users search, buy or sell products, suggest they clarify any unclear inputs and ask if they can provide more info or that's all. If that's all, confirm final query with them before calling extract_product_features(). "
...
extract_product_features_tool = Ggenai.types.FunctionDeclaration(
    ...
    description="Extract product features like name, category, price, location, condition, etc. based on input info of product",
    parameters={
        "type": "object",
        "properties": {
            ...
            "product_description": {"type": "string", "description": "Product description"}, # "Key words of prod description"
...
def chat(user_input_text):
    ...
    try:
        ...
        if hasattr(first_part, 'function_call') and first_part.function_call.name == "extract_product_features":
            ...
            try:
                function_args = {key: value for key, value in first_part.function_call.args.items()}
            ...
            # content1 = response1.text # OLD LINE
