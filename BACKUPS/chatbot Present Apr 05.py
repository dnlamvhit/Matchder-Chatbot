...
#with st.container():
with st.expander("SEARCH FOR PRODUCTS BASED ON USER-SELECTED FILTERS AND IMAGES"):
  ...
  with col2:
    ...
    print("Click data:", st.session_state['click_data'])
    ...
    if uploaded_file is not None:
        try:           
            ...
            if st.button("**Update Filters Using Uploaded Images**"):
                ...
                google_image_data = buffered.getvalue() # Get the bytes, not base64 string
                ...
                - category: best-matched category from {product_category_list}
                - description: product description in advertising style. Uncertain features are described within square brackets, using the most likely or popular features of similar products."""    
    ...
    #            """
                ...
                response = gemini.generate_content([google_prompt,  # The text prompt
                    {"mime_type": mime_type, "data": google_image_data}]) # The raw image bytes
                try:
...
def chat(user_input):
    ...
    try:
        response1 = gpt.chat.completions.create(
            model="gpt-4o-mini",
            ...
            functions=[{
                ...
                "parameters": {
                    ...
                    "properties": {
                        ...
                        "location": {"type": "string", "description": "Address of product location"},
        ...
        if response1.choices[0].finish_reason == "function_call":
            ...
            if function_name == "search_product":
                ...
                response2 = gpt.chat.completions.create(# Send the search result back to LLM to allow the model to reply to the user
                    model="gpt-4o-mini",
