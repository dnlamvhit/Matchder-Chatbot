...
#st.sidebar.markdown("<font color='darkblue'><b><p style='text-align: center; line-height: 0;'>MSCSE Capstone Project</p></b></font>", unsafe_allow_html=True)
st.sidebar.markdown("<font color='darkblue'><b><p style='text-align: center; line-height: 0;'>Professor: Dr. Glen Qin</p></b></font>", unsafe_allow_html=True)
st.sidebar.markdown("<font color='darkblue'><b><p style='text-align: center; line-height: 0;'>Student: Lam Ngoc Dao</p></b></font>", unsafe_allow_html=True)
st.sidebar.image("robo.gif", use_column_width=True)
st.sidebar.markdown("<font color='darkblue'><b><p style='text-align: center; line-height: 0;'>Welcome to CSTU chat bot!</p></b></font>", unsafe_allow_html=True)
st.sidebar.markdown("<font color='darkblue'><b><p style='text-align: center; line-height: 0;'>Version 1.0 - 04/30/2024</p></b></font>", unsafe_allow_html=True)
st.sidebar.markdown("""
<style>
.stButton > button {
  display: block;
...
""", unsafe_allow_html=True)
if st.sidebar.button("CLEAR CONTEXT HISTORY"):
...
options = st.sidebar.selectbox(
    ...
    ('UPDATE KNOWLEDGE-BASE', 'UPLOAD COURSE SCHEDULE', 'UPLOAD COURSE GRADES'),
    key='menu',
    index = st.session_state['index'] 
)
if options is not None:
    st.session_state['index'] = ('UPDATE KNOWLEDGE-BASE', 'UPLOAD COURSE SCHEDULE', 'UPLOAD COURSE GRADES').index(options)
    expander=st.sidebar    
               ...
               if file_name is not None:
                   ...
                   if st_button.button("UPLOAD"): 
                      ...
                      try:
                            ...
                            st.write(e)                                                 
            elif options == 'UPLOAD COURSE SCHEDULE':
               file_name = st.file_uploader("Select your csv schedule file", type="csv")      
               if file_name is not None:
                    st_button=st.empty()
                    if st_button.button("UPLOAD"): 
                      st_button.empty()  
                      try:
                         ...
                         st.balloons()
                         st.write("This function is being developed")  
                      except Exception as e:
                         st.write(e)
            elif options == 'UPLOAD COURSE GRADES':
               file_name = st.file_uploader("Select your csv grades file", type="csv")      
               if file_name is not None:
                   st_button=st.empty()
                   if st_button.button("UPLOAD"): 
                     st_button.empty()  
...
def get_registration(student_email):
    try:
        ...
        result = df[df["EMAIL ADDRESS"] == student_email].copy()
        result.drop(["STUDENT NAME", "EMAIL ADDRESS"], axis=1, inplace=True)        
    except Exception as e:
        result = f"Registration records not found for {student_email}"
    if isinstance(result, pd.DataFrame):
        ai_message = {"role": "assistant", "content": result.to_dict('records')}
    else:
        ai_message = {"role": "assistant", "content": result}
    st.session_state.chat_history.append(ai_message)
    
def update_grades():
    result="Please follow following steps to upload course grades:\n"
    result+="   1. Scroll down the left side bar to 'Data Management' menu\n"
    result+="   2. Seclect 'Upload grades' from the combobox below\n"
    result+="   3. Enter username and password and login\n"
    result+="   4. Browse and select your grade file\n"
    result+="   5. Select Upload button."
    ai_message = {"role": "assistant", "content": result}
    st.session_state.chat_history.append(ai_message)
...
def get_grades(student_email):
    try:
        ...
        result = df[df["student_email"] == student_email].copy()
        result.drop('student_email', axis=1, inplace=True)        
        total_unit = result["units"].sum()
        gpa = round((result["units"] * result["points"]).sum() / total_unit, 2)
    except Exception as e:
        result = f"Grades records not found for {student_email}"
    if isinstance(result, pd.DataFrame):
        #ai_message = {"role": "assistant", "content": result.to_dict('records')}
        st.session_state.chat_history.append({"role": "assistant", "content": result.to_dict('records')})
        st.session_state.chat_history.append({"role": "assistant", "content": "Total credit units = " + str(total_unit) + "\n\nCumulative GPA = " + str(gpa)})
    else:
        ai_message = {"role": "assistant", "content": result}
        st.session_state.chat_history.append(ai_message)
...
if user_input := st.chat_input("Enter a prompt here to ask me for information"):
    ...
    if OPENAI_API_KEY:
        ...
        if response.get("function_call"):  
            ...
            if function_name == 'offer_courses':
                offer_courses() 
                ...
                get_registration(function_args.get("student_email"))
            elif function_name == 'get_grades':
                get_grades(function_args.get("student_email"))                    
