    ...
    try:
        ...
        if product_data[1]:
            ...
            for item in product_data[1]:
                ...
                # Append product information to the result string
                result = "Products matching your query:\n"
                result += f"**Product Name**: {item.get('name', '')}\n"
                result += f"**Description**: {item.get('description', '')}\n"
                result += f"**Deal Type**: {item.get('deal_type', '')}\n"
                result += f"**Price**: {item.get('price', '')}\n"
                result += f"**Condition**: {item.get('condition', '')}\n"
                result += f"**Status**: {item.get('status', '')}\n"
                result += f"**Posted**: {item.get('published_at', '')}\n"
                result += f"**Allow Inspection**: {item.get('allow_inspect', '')}\n"
                result += f"**Delivery Method**: {item.get('delivery_method', '')}\n"
                result += f"**Address**: {item.get('address', '')}\n"
            return result
        else:
...
def initialize():
  if 'initialized' not in st.session_state:
    ...
    st.session_state.prompt_history = [("system", "You are Matchder marketing assistant...")]
...
if search_result: st.write(search_result) 

#st.markdown('<div class="chat-container">', unsafe_allow_html=True)
if st.session_state.chat_history:
        for chat in st.session_state.chat_history:
            with st.chat_message("user" if chat[0] == "user" else "assistant"):
                if chat[0] == "user": st.markdown("**YOU:**")
                else: st.markdown("**BOT:**")
                st.write(chat[1])
